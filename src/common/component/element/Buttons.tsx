...
const Buttons = ({className= '', icons,icon, href, title}: ButtonsProps) => {
  return (
    <Button
      size="lg"
      color="default"
      className={clsx("lg:hover:bg-neutral-100 bg-[#FF6B00] relative group lg:transition-all lg:duration-300", className)}
      as={Link}
      href={href}
      target="__blank"
    >
      <h1 className=" z-[9] flex items-center gap-2 lg:group-hover:text-black text-white lg:transition-all lg:duration-300 relative">
        <span>
        {icon && <>{icon}</>}
        </span>
        {title}
